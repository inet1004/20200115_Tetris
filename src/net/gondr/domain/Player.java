...
public class Player {
	...
	public Player(Block[][] board) {
		...
		// _┌━ 모양은 여러분이 만들어보세요.
		shape[4] = new Point2D[4][];
		shape[4][0] = getPointArray("1,-1:1,0:0,0:0,1");
		shape[4][1] = getPointArray("1,-1:1,0:0,0:0,1");
		shape[4][2] = getPointArray("1,-1:1,0:0,0:0,1");
		shape[4][3] = getPointArray("1,-1:1,0:0,0:0,1");
		
		// ─┐_ 모양은 여러분이 만들어보세요.
		shape[5] = new Point2D[4][];
		shape[5][0] = getPointArray("0,-1:0,0:1,0:1,1");
		shape[5][1] = getPointArray("0,-1:0,0:1,0:1,1");
		shape[5][2] = getPointArray("0,-1:0,0:1,0:1,1");
		shape[5][3] = getPointArray("0,-1:0,0:1,0:1,1");
		
		// ㅗ 모양은 여러분이 만들어보세요.
		shape[6] = new Point2D[4][];
		shape[6][0] = getPointArray("1,-1:1,0:0,0:1,1");
		shape[6][1] = getPointArray("1,-1:1,0:0,0:1,1");
		shape[6][2] = getPointArray("1,-1:1,0:0,0:1,1");
		shape[6][3] = getPointArray("1,-1:1,0:0,0:1,1");
	...
	public boolean down() {
		...
		if(!checkPossible()) {
			...
			Main.app.game.checkLineStatus(); //블럭을 내린후에는 현재 라인상태를 체크하도록 함.
			getNextBlock();
			draw(false); //이동후 그려주기
